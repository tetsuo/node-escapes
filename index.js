...
module.exports = function (cb) {
  ...
  stream._write = function (row, enc, next) {
    dup.write(row);
    next();
  };

  dup
    .on('cuu', function (args) {
      cursor.moveCursorBy(0, -(args[0] || 1));
    })
    .on('cud', function (args) {
      cursor.moveCursorBy(0, args[0] || 1);
    })
    .on('cuf', function (args) {
      cursor.moveCursorBy(args[0] || 1, 0);
    })
    .on('cub', function (args) {
      cursor.moveCursorBy(-(args[0] || 1), 0);
    })
    .on('cup', function (args) {
      cursor.row = args[0] || 1;
      cursor.column = args[1] || 1;
    })
    .on('sc', cursor.savePosition.bind(cursor))
    .on('rc', cursor.loadPosition.bind(cursor))
        ...
        if (arg === NONE) {
          ...
          switch (Math.floor(arg / 10)) {
            case 0:
              ...
              break;
            case 3:
              cursor.foreground = arg - 30;
              break;
            case 4:
              cursor.background = arg - 40;
              break;
          }
        }
      }
    ...
    .on('ed', function (args) {
      if (args[0] === 2) cursor.clearCanvas();
    })
    .on('text', cursor.write.bind(cursor))
    .on('finish', function () {
			cursor.trimCanvas();
			cb(cursor.canvas);
		});

  stream.on('finish', dup.end.bind(dup));

  return stream;
};

function Cursor() {
  this.canvas = new Canvas(640, MAX_HEIGHT);
...
Cursor.prototype.moveCursorBy = function (columns, rows) {
  this.column += columns;
  this.row += rows;
...
Cursor.prototype.savePosition = function () {
  this.saved = {};
  this.saved.row = this.row;
  this.saved.column = this.column;
};

Cursor.prototype.loadPosition = function () {
  this.column = this.saved.column;
  this.row = this.saved.row;
  delete this.saved;
};

Cursor.prototype.getColor = function (code, bright) {
  return this.palette[bright ? code + 8 : code];
};

Cursor.prototype.resetColor = function () {
  this.foreground = WHITE;
  this.background = BLACK;
};

Cursor.prototype.renderChar = function (charcode, foreground, background) {
...
Cursor.prototype.write = function (text) {
  var CR = 0x0d, LF = 0x0a,
      cursor = this, image_data,
      background, foreground,
      charcode, x, y, i, length;
